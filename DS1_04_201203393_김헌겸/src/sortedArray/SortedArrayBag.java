...
public class SortedArrayBag {
	...
	public Coin maxCoin()
	{
		return this._elements[this._size-1];   //정렬된 배열의 최대값은 제일 마지막 요소다
	}
	...
	{	
//		if(anElement.value() < 0 || anElement.value() > SortedArrayBag.DEFAULT_MAX_SIZE)           //실행불가!
//			return false;
		...
		{
			for(int index = 0; index<this._size; index++)             //foundIndex가 for문을 빠져나갈때 증가후 빠짐.
			{
				if(this._elements[index].value() > anElement.value()) 
				{
					foundIndex = index;
					break;
				}
			}
			for(int index = this._size; index>foundIndex; index--)
			{
				this._elements[index] = this._elements[index-1];
			}
			this._elements[foundIndex] = anElement;
			this._size++;
		}
		return true;
	}
